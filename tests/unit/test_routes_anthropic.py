...
class TestCountTokensAuthentication:
    ...
    def test_count_tokens_requires_authentication(self, test_client):
        """
        What it does: Verifies count_tokens endpoint requires authentication.
        Purpose: Ensure protected endpoint is secured.
        """
        print("Action: POST /v1/messages/count_tokens without auth...")
        response = test_client.post(
    ...
    def test_count_tokens_accepts_bearer_token(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies count_tokens endpoint accepts Bearer token.
        Purpose: Ensure OpenAI-style authentication also works here.
        """
        print("Action: POST /v1/messages/count_tokens with Bearer token...")
        with patch(
            "kiro.routes_anthropic.estimate_anthropic_request_tokens",
            return_value={"messages_tokens": 7, "tools_tokens": 0, "system_tokens": 0, "total_tokens": 7},
        ):
            response = test_client.post(
                "/v1/messages/count_tokens",
                headers={"Authorization": f"Bearer {valid_proxy_api_key}"},
...
class TestCountTokensValidation:
    ...
    def test_count_tokens_validates_missing_model(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies missing model field is rejected.
        Purpose: Ensure count_tokens requests still require a target model.
        """
        print("Action: POST /v1/messages/count_tokens without model...")
        response = test_client.post(
            "/v1/messages/count_tokens",
            headers={"x-api-key": valid_proxy_api_key},
    ...
    def test_count_tokens_validates_missing_messages(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies missing messages field is rejected.
        Purpose: Ensure count_tokens has required prompt content.
        """
        print("Action: POST /v1/messages/count_tokens without messages...")
        response = test_client.post(
            "/v1/messages/count_tokens",
            headers={"x-api-key": valid_proxy_api_key},
    ...
    def test_count_tokens_does_not_require_max_tokens(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies count_tokens works without max_tokens.
        Purpose: Match Anthropic count_tokens contract instead of Messages API contract.
        """
        print("Action: POST /v1/messages/count_tokens without max_tokens...")
        with patch(
            "kiro.routes_anthropic.estimate_anthropic_request_tokens",
            return_value={"messages_tokens": 11, "tools_tokens": 0, "system_tokens": 0, "total_tokens": 11},
        ):
            response = test_client.post(
                "/v1/messages/count_tokens",
                headers={"x-api-key": valid_proxy_api_key},
...
class TestCountTokensEndpoint:
    """Tests for /v1/messages/count_tokens endpoint behavior."""

    def test_count_tokens_returns_input_tokens(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies endpoint returns Anthropic count_tokens response shape.
        Purpose: Ensure Claude Code receives the field it expects.
        """
    ...
    def test_count_tokens_accepts_anthropic_headers(self, test_client, valid_proxy_api_key):
        """
        What it does: Verifies anthropic-version and anthropic-beta headers are accepted.
        Purpose: Preserve Claude Code / Anthropic SDK compatibility.
        """
        print("Action: POST /v1/messages/count_tokens with Anthropic headers...")
        with patch(
            "kiro.routes_anthropic.estimate_anthropic_request_tokens",
            return_value={"messages_tokens": 9, "tools_tokens": 0, "system_tokens": 0, "total_tokens": 9},
        ):
            response = test_client.post(
                "/v1/messages/count_tokens",
                headers={
        ...
        print(f"Status: {response.status_code}")
        assert response.status_code == 200
        assert response.json() == {"input_tokens": 9}
    ...
    def test_messages_endpoint_uses_post_method(self):
        ...
        for route in router.routes:
            ...
            if route.path == "/v1/messages/count_tokens":
                print(f"Route /v1/messages/count_tokens methods: {route.methods}")
                assert "POST" in route.methods
